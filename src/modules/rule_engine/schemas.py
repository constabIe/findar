...
from .enums import (
    RuleType, RuleStatus, RuleMatchStatus, RiskLevel, TransactionType,
    ThresholdOperator, TimeWindow, CompositeOperator, CacheStatus
)
...
class ThresholdRuleParams(BaseModel):
    ...
    """
    # Amount thresholds
    max_amount: Optional[float] = Field(None, description="Maximum transaction amount allowed")
    min_amount: Optional[float] = Field(None, description="Minimum transaction amount allowed")
    
    # Frequency thresholds (within time window)
    max_transactions_per_account: Optional[int] = Field(None, description="Max transactions from same account")
    max_transactions_to_account: Optional[int] = Field(None, description="Max transactions to same account")
    max_transaction_types: Optional[int] = Field(None, description="Max different transaction types per account")
    
    # Time-based thresholds
    time_window: TimeWindow = Field(TimeWindow.HOUR, description="Time window for frequency checks")
    allowed_hours_start: Optional[int] = Field(None, ge=0, le=23, description="Start of allowed transaction hours (0-23)")
    allowed_hours_end: Optional[int] = Field(None, ge=0, le=23, description="End of allowed transaction hours (0-23)")
    
    # Velocity thresholds
    max_velocity_amount: Optional[float] = Field(None, description="Maximum total amount in time window")
    
    # Geographic thresholds
    allowed_locations: Optional[List[str]] = Field(None, description="List of allowed transaction locations")
    
    # Device/IP thresholds
    max_devices_per_account: Optional[int] = Field(None, description="Max unique devices per account in time window")
    max_ips_per_account: Optional[int] = Field(None, description="Max unique IPs per account in time window")
    ...
    def validate_time_range(cls, v, values):
        ...
        if start is not None and v is not None and v <= start:
            raise ValueError("allowed_hours_end must be greater than allowed_hours_start")
        return v
...
class PatternRuleParams(BaseModel):
    ...
    """
    # Pattern definition
    min_transactions: int = Field(3, ge=1, description="Minimum number of transactions in pattern")
    max_transactions: Optional[int] = Field(None, description="Maximum number of transactions in pattern")
    time_window: TimeWindow = Field(TimeWindow.HOUR, description="Time window for pattern detection")
    
    # Amount patterns
    max_individual_amount: Optional[float] = Field(None, description="Maximum amount per transaction in pattern")
    min_individual_amount: Optional[float] = Field(None, description="Minimum amount per transaction in pattern")
    max_total_amount: Optional[float] = Field(None, description="Maximum total amount for all transactions")
    
    # Sequence patterns
    require_sequential: bool = Field(False, description="Whether transactions must be sequential in time")
    max_time_between_transactions: Optional[int] = Field(None, description="Max seconds between consecutive transactions")
    
    # Account patterns
    same_source_account: bool = Field(True, description="All transactions from same source account")
    same_destination_account: bool = Field(False, description="All transactions to same destination account")
    different_destination_accounts: bool = Field(False, description="All transactions to different accounts")
    
    # Transaction type patterns
    allowed_types: Optional[List[TransactionType]] = Field(None, description="Allowed transaction types in pattern")
    require_same_type: bool = Field(False, description="All transactions must be same type")
    
    # Geographic patterns
    same_location: bool = Field(False, description="All transactions from same location")
    different_locations: bool = Field(False, description="All transactions from different locations")
    
    # Structuring detection (classic money laundering pattern)
    detect_structuring: bool = Field(False, description="Detect structuring patterns (many small amounts)")
    structuring_threshold: Optional[float] = Field(9999.0, description="Amount threshold for structuring detection")
...
class CompositeRuleParams(BaseModel):
    ...
    # Logical structure
    operator: CompositeOperator = Field(CompositeOperator.AND, description="Primary logical operator")
    expression: Optional[str] = Field(None, description="Complex boolean expression (e.g., '(rule1 AND rule2) OR NOT rule3')")
    
    # Execution settings
    short_circuit: bool = Field(True, description="Stop evaluation on first match (for OR) or first non-match (for AND)")
    require_all_success: bool = Field(False, description="Require all referenced rules to execute successfully")
    
    # Scoring
    weight_mode: str = Field("equal", description="How to weight individual rule results (equal, priority, custom)")
    custom_weights: Optional[Dict[int, float]] = Field(None, description="Custom weights for each rule (rule_id -> weight)")
    min_weighted_score: Optional[float] = Field(None, description="Minimum weighted score for match")
...
class MLRuleParams(BaseModel):
    ...
    """
    # Model configuration
    model_name: str = Field("fraud_detection_model", description="Name of ML model to use")
    model_version: Optional[str] = Field(None, description="Specific model version")
    
    # Feature configuration
    features: List[str] = Field(default_factory=list, description="List of features to extract from transaction")
    feature_preprocessing: Dict[str, Any] = Field(default_factory=dict, description="Feature preprocessing parameters")
    
    # Scoring thresholds
    confidence_threshold: float = Field(0.5, ge=0.0, le=1.0, description="Minimum confidence score for positive match")
    
    # Model execution
    timeout_ms: int = Field(5000, gt=0, description="Model execution timeout in milliseconds")
    fallback_behavior: str = Field("allow", description="Behavior when model fails (allow/deny/skip)")
    
    # Mock configuration (temporary)
    mock_mode: bool = Field(True, description="Use mock predictions instead of real model")
    mock_confidence_range: tuple[float, float] = Field((0.1, 0.9), description="Range for mock confidence scores")
...
class RuleEvaluationRequest(BaseModel):
    ...
    rule_ids: Optional[List[int]] = Field(None, description="Specific rule IDs to evaluate (None = all active)")
    correlation_id: str = Field(description="Correlation ID for tracking")
    
    # Evaluation options
    include_context: bool = Field(True, description="Include evaluation context in results")
    include_performance: bool = Field(False, description="Include performance metrics")
    stop_on_critical_match: bool = Field(True, description="Stop evaluation on critical rule match")
...
class RuleContext(BaseModel):
    ...
    # Transaction context
    transaction_data: Dict[str, Any] = Field(description="Transaction data used in evaluation")
    
    # Rule-specific context
    threshold_values: Optional[Dict[str, Any]] = Field(None, description="Calculated threshold values")
    pattern_matches: Optional[List[Dict[str, Any]]] = Field(None, description="Pattern match details")
    composite_results: Optional[Dict[int, bool]] = Field(None, description="Results from composite rule components")
    ml_features: Optional[Dict[str, float]] = Field(None, description="ML model features and values")
    
    # Historical context
    related_transactions: Optional[List[UUID]] = Field(None, description="Related transactions considered")
    account_history: Optional[Dict[str, Any]] = Field(None, description="Account history summary")
...
class RuleEvaluationResult(BaseModel):
    ...
    confidence_score: Optional[float] = Field(None, ge=0.0, le=1.0, description="Confidence score (for ML rules)")
    risk_level: RiskLevel = Field(description="Assessed risk level")
    
    # Context and explanation
    context: Optional[RuleContext] = Field(None, description="Detailed evaluation context")
    match_reason: Optional[str] = Field(None, description="Human-readable explanation of match/non-match")
...
class TransactionEvaluationResult(BaseModel):
    ...
    rule_results: List[RuleEvaluationResult] = Field(description="Results from individual rules")
    ...
    critical_rules_matched: int = Field(description="Number of critical rules that matched")
...
class RuleCreateRequest(BaseModel):
    ...
    params: Union[ThresholdRuleParams, PatternRuleParams, CompositeRuleParams, MLRuleParams] = Field(description="Rule parameters")
    ...
    def validate_params_match_type(cls, params, info):
        ...
        elif rule_type == RuleType.PATTERN and not isinstance(params, PatternRuleParams):
            raise ValueError("Pattern rules must use PatternRuleParams")
        elif rule_type == RuleType.COMPOSITE and not isinstance(params, CompositeRuleParams):
...
class RuleUpdateRequest(BaseModel):
    ...
    params: Optional[Union[ThresholdRuleParams, PatternRuleParams, CompositeRuleParams, MLRuleParams]] = Field(None, description="Rule parameters")
...
class HotReloadRequest(BaseModel):
    ...
    rule_ids: Optional[List[int]] = Field(None, description="Specific rule IDs to reload (None = all)")
    force_refresh: bool = Field(False, description="Force refresh even if cached version is current")
...
class HotReloadResponse(BaseModel):
    ...
    cache_statistics: List[CacheStatusResponse] = Field(description="Updated cache statistics")
