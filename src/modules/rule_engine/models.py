...
class Transaction(SQLModel, table=True):
    ...
    status: TransactionStatus = Field(default=TransactionStatus.QUEUED, description="Transaction processing status")
    ...
    merchant_id: Optional[str] = Field(default=None, description="Merchant identifier for payments")
    ...
    device_id: Optional[str] = Field(default=None, description="Device used for transaction")
    ip_address: Optional[str] = Field(default=None, description="IP address of transaction origin")
...
class Rule(SQLModel, table=True):
    ...
    priority: int = Field(default=0, description="Rule execution priority (higher = first)")
    critical: bool = Field(default=False, description="Critical rule for short-circuiting")
    ...
    execution_count: int = Field(default=0, description="Number of times rule was executed")
    ...
    average_execution_time_ms: Optional[float] = Field(default=None, description="Average execution time in milliseconds")
...
class RuleExecution(SQLModel, table=True):
    ...
    context: Dict[str, Any] = Field(default_factory=dict, sa_type=JSON, description="Rule execution context")
...
class RuleCache(SQLModel, table=True):
    ...
    last_accessed_at: Optional[datetime] = Field(default=None, description="Last cache access timestamp")