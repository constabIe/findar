...
async def enqueue_transaction(redis_client: Redis, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    4. Increments Prometheus metrics
    ...
    logger.info("Starting transaction enqueue process", 
                event="enqueue_start",
    ...
    try:
        ...
        try:
            ...
            logger.debug("Celery task triggered successfully", 
                        event="celery_task_triggered",
                        task_name="transactions.consume")
        except Exception as e:
            logger.warning("Failed to trigger Celery task", 
        ...
        logger.error("Transaction enqueue failed",
                    event="enqueue_error",
                    error=str(e),
