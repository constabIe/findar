...
class UserCreate(BaseModel):
    ...
    password: str = Field(
        ..., min_length=1, description="User's password (minimum 6 characters)"
    )
    telegram_alias: str = Field(
        ..., min_length=3, description="Telegram username (without @)"
    )
...
class UserResponse(BaseModel):
    ...
    telegram_alias: str = Field(..., description="Telegram username")
    telegram_id: Optional[int] = Field(
        default=None, description="Telegram user ID (if bot started)"
    )
