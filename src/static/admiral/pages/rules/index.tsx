...
const Rules: React.FC = () => {
  ...
  const handleToggleRule = async (ruleId: string, currentValue: boolean) => {
    try {
      ...
      showNotification(
        `Rule ${currentValue ? "deactivated" : "activated"} successfully!`,
        "success"
      )
    } catch (err: any) {
            ...
            {loading ? (
              ...
              <>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                      <tr style={{ borderBottom: "2px solid #ddd" }}>
                        ...
                        <th style={{ padding: "12px 8px", textAlign: "left" }}>
                          Max Transaction Types
                        </th>
                        <th style={{ padding: "12px 8px", textAlign: "left" }}>
                          Max Transactions/Account
                        </th>
                        <th style={{ padding: "12px 8px", textAlign: "left" }}>
                          Max Transactions to Account
                        </th>
                        <th style={{ padding: "12px 8px", textAlign: "left" }}>
                          Max Transactions/IP
                        </th>
                    ...
                    <tbody>
                      {paginatedRules.map((rule) => (
                        ...
                        >
                            ...
                            >
                              <input
                                ...
                                onChange={() =>
                                  handleToggleRule(
                                    rule.id,
                                    getRuleApplyState(rule.id, rule.enabled)
                                  )
                                }
                                style={{
                                  width: "40px",
                                  height: "20px",
                                  appearance: "none",
                                  backgroundColor: getRuleApplyState(rule.id, rule.enabled)
                                    ? "#4caf50"
                                    : "#ccc",
                                  borderRadius: "10px",
                                  position: "relative",
                          ...
                          <td style={{ padding: "12px 8px" }}>
                            {rule.params.max_devices_per_account || "-"}
                          </td>
                          <td style={{ padding: "12px 8px" }}>
                            {rule.params.max_ips_per_account || "-"}
                          </td>
                          <td style={{ padding: "12px 8px" }}>
                            {rule.params.max_velocity_amount || "-"}
                          </td>
                          <td style={{ padding: "12px 8px" }}>
                            {rule.params.max_transaction_types || "-"}
                          </td>
                          <td style={{ padding: "12px 8px" }}>
                            {rule.params.max_transactions_per_account || "-"}
                          </td>
                          <td style={{ padding: "12px 8px" }}>
                            {rule.params.max_transactions_to_account || "-"}
                          </td>
                          <td style={{ padding: "12px 8px" }}>
                            {rule.params.max_transactions_per_ip || "-"}
                          </td>
                          <td style={{ padding: "12px 8px" }}>{rule.params.period || "-"}</td>
                          <td style={{ padding: "12px 8px" }}>{rule.params.count || "-"}</td>
                          <td style={{ padding: "12px 8px" }}>
                            {rule.params.amount_ceiling || "-"}
                          </td>
                          <td style={{ padding: "12px 8px" }}>
                            {rule.params.same_recipient !== undefined ? (
                              <span
                                style={{
                                  ...
                                  borderRadius: "4px",
                                  fontSize: "12px",
                                  backgroundColor: rule.params.same_recipient
                                    ? "#d4edda"
                                    : "#f8d7da",
                            ...
                            {rule.params.composite_operator ? (
                              <span
                                style={{
                                  ...
                                  fontWeight: "bold",
                                  backgroundColor:
                                    rule.params.composite_operator === "AND"
                                      ? "#e3f2fd"
                                      : rule.params.composite_operator === "OR"
                                        ? "#fff3e0"
                                        : "#ffebee",
                                  color:
                                    rule.params.composite_operator === "AND"
                ...
                >
                  ...
                  <div>
                    <span style={{ marginRight: "12px" }}>Total: {filteredRules.length} rules</span>
                  </div>
                </div>
              </>
            )}
