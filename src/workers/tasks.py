    ...
    try:
        ...
        try:
            ...
            if "BUSYGROUP" not in msg:
                logger.error("Failed to create consumer group", event="consumer_group_error", error=msg)
                raise
        ...
        while True:
            try:
                ...
                if not messages:
                    logger.debug("No messages available, continuing", event="no_messages")
                    break
                ...
                for _, entries in messages:
                    for entry_id, fields in entries:
                        ...
                        logger.debug("Processing transaction message",
        ...
        logger.info("Transaction consumption completed",
        ...
        logger.error("Critical error in transaction consumption",
...
def _persist_transaction(session_maker, payload: Dict[str, Any]) -> None:
    ...
    async def _async_persist():
        async with session_maker() as session:  # type: AsyncSession
            ...
            logger.debug("Transaction persisted to database",
