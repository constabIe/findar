...
def get_time_window_key(time_window: TimeWindow, timestamp: Optional[datetime] = None) -> str:
    """
    ...
    if time_window in (TimeWindow.MINUTE, TimeWindow.FIVE_MINUTES, 
                       TimeWindow.TEN_MINUTES, TimeWindow.FIFTEEN_MINUTES,
                       TimeWindow.THIRTY_MINUTES):
        # Minute-level granularity
        return timestamp.strftime("%Y-%m-%d-%H-%M")
    elif time_window in (TimeWindow.HOUR, TimeWindow.SIX_HOURS, TimeWindow.TWELVE_HOURS):
...
async def track_transaction_metrics(
    ...
    """
    logger.info(
        f"Tracking transaction metrics in batch",
    ...
    txn_count = await increment_transaction_count(redis, from_account, time_window, ttl_seconds=ttl)
    velocity = await increment_velocity(redis, from_account, amount, time_window, ttl_seconds=ttl)
    unique_types = await add_transaction_type(redis, from_account, transaction_type, time_window, ttl_seconds=ttl)
    
    # Track to_account metrics
    to_account_count = await increment_to_account_count(redis, to_account, time_window, ttl_seconds=ttl)
    
    # Track optional metrics
    unique_devices = 0
    if device_id:
        unique_devices = await add_device_to_account(redis, from_account, device_id, time_window, ttl_seconds=ttl)
    
    unique_ips = 0
    if ip_address:
        unique_ips = await add_ip_to_account(redis, from_account, ip_address, time_window, ttl_seconds=ttl)
    ...
    logger.info(
        f"Transaction metrics tracked successfully",
