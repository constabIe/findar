...
from redis.exceptions import RedisError, ConnectionError as RedisConnectionError, TimeoutError, ResponseError
...
async def get_async_redis() -> AsyncConnectionPool:
    ...
    if _async_redis_pool is None:
        try:
            ...
            logger.info("Creating async Redis connection pool", url=redis_url.split('@')[-1] if '@' in redis_url else redis_url)
...
def get_sync_redis() -> SyncRedis:
    ...
    if _sync_redis_client is None:
        try:
            redis_url = get_redis_url()
            logger.info("Creating sync Redis client", url=redis_url.split('@')[-1] if '@' in redis_url else redis_url)
...
async def get_async_redis_dependency() -> AsyncGenerator[AsyncRedis, None]:
    ...
    try:
        ...
        logger.debug("Redis session started", extra={
            "component": "redis_client",
            "event": "session_start"
        })
        yield client
    except Exception as e:
        logger.error("Redis session error", extra={
            "component": "redis_client",
            "event": "session_error",
            "error": str(e)
        })
        ...
        logger.debug("Redis session ended", extra={
            "component": "redis_client",
            "event": "session_end"
        })
...
def get_sync_redis_dependency() -> Generator[SyncRedis, None, None]:
    ...
    try:
        logger.debug("Sync Redis session started", extra={
            "component": "redis_client",
            "event": "sync_session_start"
        })
        yield client
    except Exception as e:
        logger.error("Sync Redis session error", extra={
            "component": "redis_client",
        ...
        logger.debug("Sync Redis session ended", extra={
            "component": "redis_client",
            "event": "sync_session_end"
        })
...
async def close_redis_connections():
    ...
    if _async_redis_pool:
        logger.info("Closing async Redis connection pool", extra={
            "component": "redis_client",
            "event": "async_pool_close"
        })
        try:
    ...
    if _sync_redis_client:
        logger.info("Closing sync Redis connection", extra={
            "component": "redis_client", 
            "event": "sync_close"
        })
        try:
