...
def get_async_engine():
    ...
    if _async_engine is None:
        ...
        try:
            ...
            logger.info("Database engine created successfully", event="engine_created")
...
def get_async_session_maker():
    ...
    if _async_session_maker is None:
        try:
            ...
            logger.info(
                "Async session maker created successfully",
                event="session_maker_created",
            )

        except Exception as e:
            raise DatabaseError(
                "Failed to create async session maker", operation="create_session_maker"
            ) from e
...
async def get_async_session() -> AsyncGenerator[AsyncSession, None]:
    ...
    try:
        ...
        raise DatabaseError(
            "Failed to get session maker", operation="get_session_maker"
        ) from e
    ...
    try:
        async with session_maker() as session:
            try:
                logger.debug(
                    "Async database session started", event="async_session_start"
                )
                print("Async session started")
                yield session
            except SQLAlchemyError as e:
                ...
                raise DatabaseError(
                    "Database session operation failed", operation="session_operation"
                ) from e
            except Exception as e:
                logger.error(
                ...
                logger.debug("Async database session closed", event="async_session_end")
    except OperationalError as e:
        logger.error(
            "Database connection error", event="connection_error", error_message=str(e)
        )
        raise DatabaseError(
            "Database connection failed", operation="create_session"
        ) from e
    except Exception as e:
        logger.error(
        ...
        raise DatabaseError(
            "Failed to create database session", operation="create_session"
        ) from e
...
async def close_async_engine():
    ...
    if _async_engine:
        logger.info(
            "Closing async database engine",
            extra={"component": "sql_engine", "event": "engine_close"},
        )
