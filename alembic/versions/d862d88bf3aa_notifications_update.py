...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_notification_channel_configs_channel"),
        "notification_channel_configs",
        ["channel"],
        unique=True,
    )
    op.create_index(
        op.f("ix_notification_channel_configs_enabled"),
        "notification_channel_configs",
        ["enabled"],
        unique=False,
    )
    op.create_table(
        ...
        sa.Column(
            "channel",
            postgresql.ENUM("EMAIL", "TELEGRAM", name="notificationchannel", create_type=False),
            nullable=False,
        ),
        ...
        sa.Column(
            "custom_fields", postgresql.JSON(astext_type=sa.Text()), nullable=True
        ),
        sa.Column("description", sqlmodel.AutoString(), nullable=True),
    ...
    op.create_index(
        "idx_template_channel_enabled",
        "notification_templates",
        ["channel", "enabled"],
        unique=False,
    )
    op.create_index(
        "idx_template_priority", "notification_templates", ["priority"], unique=False
    )
    op.create_index(
        "idx_template_type", "notification_templates", ["type"], unique=False
    )
    op.create_index(
        op.f("ix_notification_templates_enabled"),
        "notification_templates",
        ["enabled"],
        unique=False,
    )
    ...
    op.create_index(
        "idx_delivery_scheduled",
        "notification_deliveries",
        ["scheduled_at"],
        unique=False,
    )
    op.create_index(
        "idx_delivery_status_created",
        "notification_deliveries",
        ["status", "created_at"],
        unique=False,
    )
    op.create_index(
        "idx_delivery_transaction",
        "notification_deliveries",
        ["transaction_id"],
        unique=False,
    )
    ...
    op.create_index(
        op.f("ix_notification_deliveries_template_id"),
        "notification_deliveries",
        ["template_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_notification_deliveries_transaction_id"),
        "notification_deliveries",
        ["transaction_id"],
        unique=False,
    )
    ...
    op.add_column(
        "users", sa.Column("email_notifications_enabled", sa.Boolean(), nullable=False)
    )
    op.add_column(
        "users",
    ...
    op.create_foreign_key(
        "fk_users_email_template_id", "users", "notification_templates", ["email_template_id"], ["id"]
    )
    op.create_foreign_key(
        "fk_users_telegram_template_id", "users", "notification_templates", ["telegram_template_id"], ["id"]
    )
...
def downgrade() -> None:
    ...
    op.drop_constraint("fk_users_telegram_template_id", "users", type_="foreignkey")
    op.drop_constraint("fk_users_email_template_id", "users", type_="foreignkey")
    
    # 2. Затем удаляем колонки из users
    ...
    # 4. Удаляем notification_templates (ПОСЛЕ зависимых таблиц)
    op.drop_index(
        op.f("ix_notification_templates_name"), table_name="notification_templates"
    )
    op.drop_index(
        op.f("ix_notification_templates_enabled"), table_name="notification_templates"
    )
    op.drop_index("idx_template_priority", table_name="notification_templates")
    op.drop_table("notification_templates")
    
    # 5. Удаляем notification_channel_configs
    op.drop_table("notification_channel_configs")
    
    # 6. В КОНЦЕ удаляем ENUM типы
