...
def upgrade() -> None:
    ...
    sa.Column('status', sa.Enum('PENDING', 'APPROVED', 'FLAGGED', 'FAILED', name='taskstatus'), nullable=False),
    sa.Column('retry_count', sa.Integer(), nullable=False),
    sa.Column('max_retries', sa.Integer(), nullable=False),
    sa.Column('error_type', sa.Enum('VALIDATION_ERROR', 'DATABASE_ERROR', 'RULE_ENGINE_ERROR', 'NOTIFICATION_ERROR', 'TIMEOUT_ERROR', 'UNKNOWN_ERROR', name='errortype'), nullable=True),
    ...
    op.create_index('idx_queue_celery_task', 'queue_tasks', ['celery_task_id'], unique=False)
    op.create_index('idx_queue_correlation', 'queue_tasks', ['correlation_id'], unique=False)
    op.create_index('idx_queue_status_created', 'queue_tasks', ['status', 'created_at'], unique=False)
    op.create_index('idx_queue_transaction', 'queue_tasks', ['transaction_id'], unique=False)
    op.create_index(op.f('ix_queue_tasks_celery_task_id'), 'queue_tasks', ['celery_task_id'], unique=False)
    op.create_index(op.f('ix_queue_tasks_correlation_id'), 'queue_tasks', ['correlation_id'], unique=True)
    op.create_index(op.f('ix_queue_tasks_status'), 'queue_tasks', ['status'], unique=False)
    op.create_index(op.f('ix_queue_tasks_transaction_id'), 'queue_tasks', ['transaction_id'], unique=False)
    op.alter_column('rules', 'name',
               existing_type=sa.VARCHAR(),
               nullable=True)
    op.alter_column('rules', 'params',
